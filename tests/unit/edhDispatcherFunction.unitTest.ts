...
describe('edhDispatcher function', () => {
  describe('if the event has no records', () => {
    it('should return undefined', async () => {
      expect.assertions(1);
      const result = await edhDispatcher({ something: 'not records' } as unknown as SQSEvent, mockContext, () => {
      });
  ...
  describe('with good event', () => {
    it('invokes the dispatch service with the right body and target', async () => {
      ...
      jest.spyOn(DispatchService.prototype, 'processSQSRecord').mockImplementation(processMock);
    ...
    describe('and ProcessEvent returns a rejection', () => {
      it('throws the error upwards', async () => {
        ...
        const processMock = jest.spyOn(DispatchService.prototype, 'processSQSRecord').mockReturnValue(Promise.reject(error));
        expect.assertions(2);
        try {
          await edhDispatcher(event, mockContext, () => {
          });
    ...
    describe('and ProcessEvent returns a process resolve', () => {
      it('returns the resolution value', async () => {
        ...
        const processMock = jest.spyOn(DispatchService.prototype, 'processSQSRecord').mockReturnValue(Promise.resolve());
        expect.assertions(2);
