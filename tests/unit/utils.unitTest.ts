  ...
  describe("getTargetFromSourceARN", () => {
    describe("when ARN has a matching config", () => {
      it("gets target from config properly", () => {
        const expected =
          Configuration.getInstance().getTargets()["test-results"];
        const target = getTargetFromSourceARN(
          "arn:aws:sqs:eu-west-1:006106226016:cvs-test-results-cvsb-10773-queue"
        );
    ...
    describe("when ARN has no matching config", () => {
      it("throws error", () => {
        try {
          getTargetFromSourceARN(
            "arn:aws:dynamodb:horse-east-8:00626016:table/cvs-cvsb-xxx-NO-MATCHING/stream/2024-03-30T15:55:39.197"
          );
        } catch (e) {
